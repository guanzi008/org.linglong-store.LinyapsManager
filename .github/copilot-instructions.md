# LinyapsManager Copilot 指南
这个项目是一个dbus转发命令行输出的项目
本仓库实现了 `org.linglong-store.LinyapsManager`，这是一个通过封装 `ll-cli` 工具来管理玲珑应用的 D-Bus 服务。它包含一个服务端守护进程和一个 CLI 客户端 (`linyapsctl`)。
